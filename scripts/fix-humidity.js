...
async function main() {
  console.log('Restoring ACTUAL NOAA Relative Humidity data...');
  const filePath = path.resolve(__dirname, '..', 'data', 'NOAA_Relative_Humidity.xlsx');
  const wb = XLSX.readFile(filePath);
  const data = XLSX.utils.sheet_to_json(wb.Sheets[wb.SheetNames[0]], { header: 1 });

  const stations = [];
  // The structure seems to have actual text headers and then data. Let's start at likely row 3 or 4.
  for (let i = 3; i < data.length; i++) {
    const row = data[i];
    if (!row || row.length < 29) continue;
    
    // Check if index 2 is a year range like "1948-2018"
    ...
    // Column 27 is Annual Morning, Column 28 is Annual Afternoon
    const m = parseInt(row[27]);
    const a = parseInt(row[28]);
    
    if (!isNaN(m) && !isNaN(a)) {
      const avg_humidity = Math.round((m + a) / 2.0);
      stations.push({ name: parts[0].trim(), stateAbbr, avg_humidity });
    }
  }
  console.log(`Parsed ${stations.length} humidity stations.`);

  // Load NOAA normal coords directly from our mly_inventory.txt
  ...
  for (const c of cities) {
    ...
    if (closest) updates.push({ fips_code: c.fips_code, avg_humidity: closest.avg_humidity });
  }

  console.log(`Applying ${updates.length} raw humidity values...`);
  for (let i = 0; i < updates.length; i += 100) {
    const chunk = updates.slice(i, i + 100);
    await Promise.all(chunk.map(u => supabase.from('city_climate').update({ avg_humidity: u.avg_humidity }).eq('fips_code', u.fips_code)));
  }
  console.log('✅ Humidity restored to exact authentic NOAA values.');
}
